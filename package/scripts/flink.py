...
class Master(Script):
  def install(self, env):
    ...
    if params.setup_prebuilt:
      ...
      if os.path.exists(params.flink_release_file):
        Execute('tar -zxvf '+params.flink_release_file+' -C ' + params.flink_install_dir + ' >> ' + params.flink_log_file, user=params.flink_user)
        Execute('mv '+params.flink_install_dir+'/*/* ' + params.flink_install_dir, user=params.flink_user)
        
      else:
        Execute('wget '+params.flink_download_url+' -O '+params.flink_release_file+' -a '  + params.flink_log_file, user=params.flink_user)
        Execute('tar -zxvf '+params.flink_release_file+' -C ' + params.flink_install_dir + ' >> ' + params.flink_log_file, user=params.flink_user)
        Execute('mv '+params.flink_install_dir+'/*/* ' + params.flink_install_dir, user=params.flink_user)
  ...
  def stop(self, env):
    ...
    self.set_conf_bin(env)  
    Execute(format("{params.bin_dir}/historyserver.sh stop 2>/dev/null"), user=params.flink_user)
  ...
  def start(self, env):
    ...
    cmd = format("export HADOOP_CONF_DIR={hadoop_conf_dir}; export HADOOP_CLASSPATH={hadoop_classpath}; {params.bin_dir}/historyserver.sh start")
